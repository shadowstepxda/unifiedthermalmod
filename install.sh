...
set_permissions() {
  ...
  set_perm $UNITY/system/etc/thermal-engine.conf 0 0 0644
}

# Custom Variables for Install AND Uninstall - Keep everything within this function - runs before uninstall/install
unity_custom() {
  if ! device_check "angler" && ! device_check "Nexus6P"; then
    abort "This mod is only intended to be used on the Nexus 6P. Aborting!"
  fi
}
